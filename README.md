...
このプロジェクトを始める前に、以下の環境が揃っているか確認してください。

### 1. 必要なツールのインストール確認

以下のコマンドを実行して、各ツールがインストール済みか確認：

```bash
node --version      # Node.js 22 以上が必要
npm --version
git --version
claude doctor       # Claude Code がインストール済みか確認
```
...
| **npm**              | -          | パッケージマネージャー                      |
| **Git**              | -          | バージョン管理                              |
| **VSCode**           | 最新       | テキストエディタ（推奨）                    |
| **Claude Code 拡張** | -          | VSCode 拡張機能（下記参照）                 |
| **ブラウザ**         | Chrome推奨 | アプリケーション動作確認用                  |
| **Claude Code CLI**  | -          | ターミナルから Claude Code を起動するツール |

### 3. Claude Code のインストール

Claude Code をまだインストールしていない場合は、[公式インストールガイド](https://code.claude.com/docs/ja/setup#標準インストール)を参照してインストールしてください。

インストール後、以下のコマンドで環境確認：
...
### 4. Claude Code のセットアップ（ハンズオン当日まで不要）

Claude Code CLI をインストールしておいてください。API キーはハンズオン当日に配布します。
...
**注**: 当日まで API キーがないため、`claude doctor` がすべてチェック通りにならなくても大丈夫です。

### 5. アプリケーションの動作確認（必須）

本セクションで、アプリケーションが正常に起動することを確認します。
...
**ダッシュボードが正常に表示されれば、環境セットアップは成功です！**

#### 5-6. サーバーの停止

確認が終わったら、ターミナルで `Ctrl+C` を押してサーバーを停止できます。
